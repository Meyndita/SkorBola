...
public class MatchActivity extends AppCompatActivity  {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        homeName = findViewById(R.id.txt_home);//nama team
        awayName = findViewById(R.id.txt_away);
        homeLogo = findViewById(R.id.home_logo);//gambar
        awayLogo = findViewById(R.id.away_logo);
        awayScore = findViewById(R.id.score_away);//score
        homeScore = findViewById(R.id.score_home);
        homeScoreName = findViewById(R.id.resultNameHome);//nama pecetak gol
        awayScoreName = findViewById(R.id.resultNameAway);

        Bundle bundle = getIntent().getExtras();
        //mengambil data nama team dengan variabel homeTeam dan awayTeam
        homeTeam = bundle.getString("inputHome");
        awayTeam = bundle.getString("inputAway");

        if (bundle != null){
            //mengambil data gambar dengan variabel uri
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        //request kode 1 adalah hometeam
        if (requestCode == 1) {
            if (resultCode == RESULT_OK) {
                scoreHome += 1;
                //menampilkan homescore dengan mengambil data dari scoreHome
                homeScore.setText(String.valueOf(scoreHome));
                //menerima nama pencetak gol
                ...
                //menampilkan pencetak lama dan baru
    ...
    public void handleCekHasil(View view) {
        if (scoreHome > scoreAway){
            ...
            scorerName = "Nama Pencetak Gol : \n" + homeScoreName.getText().toString();


        } else if(scoreHome < scoreAway){
             result = String.valueOf(scoreHome) + " - " + String.valueOf(scoreAway);
             message = awayTeam + " adalah Pemenang";
            scorerName = "Nama Pencetak Gol : \n" + awayScoreName.getText().toString();
